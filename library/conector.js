  ...
  async anuConector(chatUpdate, conn, m, store) {
    ...
    try {
      ...
      try {
        ...
        if (user.name !== anubis.getName(m.sender)) user.name = anubis.getName(m.sender);
        ...
        if (m.isGroup) {
          ...
          if (chats.name !== gcmeta.subject) chats.name = gcmeta.subject
      ...
      const isOwner = m.isGroup ? groupMetadata.owner.includes(m.sender) : false;
    ...
    try {
      ...
      for (let num of participants) {
        ...
        if (gcUpdate.action == 'add') {
          anubis.sendMessage(gcUpdate.id, { image: { url: pUser }, mentions: [num], caption: `Welcome To ${metadata.subject} @${num.split("@")[0]}` })
        } else if (gcUpdate.action == 'remove') {
          anubis.sendMessage(gcUpdate.id, { image: { url: pUser }, mentions: [num], caption: `@${num.split("@")[0]} Leaving To ${metadata.subject}` })
        } else if (gcUpdate.action == 'promote') {
          anubis.sendMessage(gcUpdate.id, { image: { url: pUser }, mentions: [num], caption: `@${num.split('@')[0]} Promote From ${metadata.subject}` })
        } else if (gcUpdate.action == 'demote') {
          anubis.sendMessage(gcUpdate.id, { image: { url: pUser }, mentions: [num], caption: `@${num.split('@')[0]} Demote From ${metadata.subject}` })
        }
      }
    } catch (err) {
      console.err(err);
...
global.dfail = (type, m, anubis) => {
  let msg = {
    ...
    isPremium: "Limit habis!,\nBeli limit kelipatan 500/15k! ke owner\nAtau Beli Premium ke owner 30k/bulan",
