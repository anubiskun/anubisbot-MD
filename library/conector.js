    ...
    async anuConector(chatUpdate, conn, m, store) {
      ...
      try {
        ...
        try {
          ...
          if (m.isGroup) {
            if (typeof anubis.db.data.database[m.chat] !== "object")
              anubis.db.data.database[m.chat] = {};
          ...
          if (!("antilink" in setting)) setting.antilink = true;
        ...
        const isJadibot = global["jadibot-" + m.sender.split("@")[0]]
          ? global["jadibot-" + m.sender.split("@")[0]].status
          : false;
