                 ...
                 try {
                     if (/tiktok/.test(uri.href)) {
                         ...
                         if (tiktok.status){
                             ...
                             let tt = await fetchJson('https://api.lolhuman.xyz/api/tiktok?apikey=lolirham123456&url=' + encodeURIComponent(uri.href))
                             if (tt.status === 200 && typeof tt.result == 'object' && typeof tt.result.link == 'string') anubis.sendVideo(m.chat, tt.result.link, `「 TIKTOK DOWNLOADER 」\n*USERNAME* : ${tt.result.author.username}\n*Name* : ${tt.result.author.nickname}\n*CAPTION* : ${tt.result.title}\n*Duration* : ${durasiConverter(tt.result.duration)}\n*Play Count* : ${tt.result.statistic.play_count}\n*Like Count* : ${tt.result.statistic.like_count}\n*Share Count* : ${tt.result.statistic.share_count}\n*Comment Count* : ${tt.result.statistic.comment_count}`, m, tt.result.thumbnail)
                         }
                     } else {
