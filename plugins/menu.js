...
`,
  ...
  let anuplug = async (m, anubis, {  usedPrefix, args, command, botNumber, pushname }) => {
    ...
    let hiden = ['all', 'sticker', 'downloader', 'tools', 'group', 'main', 'inject', 'owner', 'anubis']
    if (!hiden.includes(teks)) teks = 'null'
    if (teks == 'all') tags = {
      'sticker': 'Sticker',
    ...
    try {
      ...
      let name = anubis.db.data.users[m.sender].name ? pushname : ''
